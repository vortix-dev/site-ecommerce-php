<?php 
    ...
    function display_category($cat_id){
        global $con;
        $query = "SELECT * FROM categories WHERE status=1";
        if($cat_id != ''){ 
            $query = "SELECT * FROM categories WHERE id='$cat_id' AND status=1";
        }
    ...
    function get_product($cat_id = '', $prod_id = ''){
        global $con;
        $query = "SELECT * FROM product WHERE status=1 ORDER BY rand() LIMIT 12";
    ...
    function product_related(){
        global $con;
        $cat_id =$_GET['id_cat'];
        $query = "SELECT * FROM product WHERE cat_name='$cat_id' AND status=1 ORDER BY rand() LIMIT 4";
        return $result = mysqli_query($con,$query);
    }
